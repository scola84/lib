...
export class ServerTaskFilter extends Worker {
  act (box, data) {
    if (isArray(data) === false) {
      throw new Error('400 [engine] Data is not array')
    }
  ...
  filterTask (data) {
    if (isPlainObject(data) === false) {
      throw new Error('400 [engine] Task is not an object')
    }

    if (isString(data.queue) === false) {
      throw new Error('400 [engine] Queue name is not a string')
    }

    if (isString(data.name) === false) {
      throw new Error('400 [engine] Task name is not a string')
    }

    if (['none', 'return', 'stream'].indexOf(data.result) === -1) {
      throw new Error('400 [engine] Task result type is invalid')
    }
